...
/**
 * 代理店経営ダッシュボード スマホ上部の日時・天気カード
 *
 * - 所在地: allowedEmails.address から都道府県＋市区町村を抽出
 *   （取得失敗時は「東京都」にフォールバック）
 * - 天気API: Open-Meteo（無料・APIキー不要）
 *   geocoding: https://geocoding-api.open-meteo.com/v1/search
 *   forecast : https://api.open-meteo.com/v1/forecast
 * - 1分ごとに時刻のみ更新、天気は所在地変更時のみ再取得
 *
 * 表示: スマホのみ（md:hidden）
 */
...
// 住所から「都道府県+市区町村」を抽出（geocoding 精度向上のため）
function extractCityish(address) {
  if (!address) return null
  const s = String(address).normalize('NFKC').trim()
  const m = s.match(/^([^0-9]{2,4}[都道府県])([^0-9]+?[市区町村郡])/)
  if (m) return m[1] + m[2]
  const pref = s.match(/^([^0-9]{2,4}[都道府県])/)
  return pref?.[1] || null
}
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const own = snap.docs.find((d) => (d.data().address || '').trim())
        const addr = own?.data().address || ''
        const place = extractCityish(addr)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const fcUrl = `https://api.open-meteo.com/v1/forecast?latitude=${r.latitude}&longitude=${r.longitude}&daily=weather_code,temperature_2m_max,temperature_2m_min&timezone=Asia/Tokyo&forecast_days=2`
  ...
  return (
    ...
    <div className="md:hidden">
      <div className="rounded-2xl border border-rose-100 bg-gradient-to-br from-rose-50 via-white to-sky-50 p-4 shadow-sm">
        ...
        <div className="mt-1 text-xs text-gray-500">{weather?.place || placeQuery}</div>
